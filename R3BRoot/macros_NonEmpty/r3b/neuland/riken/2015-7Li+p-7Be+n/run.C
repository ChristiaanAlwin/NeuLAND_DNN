...
void Digitize(const RunConfig& cfg)
{
    ...
    tacquila->SetExperimentalDataIsCorrectedForSaturation(false);

    auto digitizer = new R3BNeulandDigitizer(tacquila);

    // Hit energy threshold
    digitizer->AddFilter([=](const R3BNeulandHit& hit) { return hit.GetE() > cfg.thresh; });

    // ±40mrad radial acceptance
    const Double_t radialAcceptance = 1080. * TMath::Tan(0.040);
    digitizer->AddFilter([=](const R3BNeulandHit& hit) { return hit.GetPosition().Perp() < radialAcceptance; });

    // velocity cut
    digitizer->AddFilter([=](const R3BNeulandHit& hit) {
        return hit.GetBeta() < cfg.velocityAcceptanceMax && hit.GetBeta() > cfg.velocityAcceptanceMin;
    });
