...
int main()
{
    ...
    fann_train_on_file(ann,"../TrainingSets/FANN_TrainingData_BERT_symSIGMOID.txt",200,1,1e-6);
    // 1) the network.
    // 2) string describing the txt-file containing the data.
    // 3) max. number of epochs.
    // 4) number of epochs between reports in screen.
    // 4) desired error for fitting.
    
    // Save the network:
    fann_save(ann,"./BabyNetwork.net");
