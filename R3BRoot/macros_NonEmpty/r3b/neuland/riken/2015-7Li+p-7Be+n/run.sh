...
if [ -z "$1" ]; then
    ...
    echo "Operations: simu digi eval plot"
    exit
fi
OPERATION=${1}
...
if [ "${OPERATION}" == "simu" ]; then
    # {FTFP_BERT,FTFP_BERT_HP,FTFP_INCLXX,FTFP_INCLXX_HP,FTF_BIC,QBBC,QGSP_BERT,QGSP_BERT_HP,QGSP_BIC,QGSP_BIC_HP,QGSP_FTFP_BERT,QGSP_INCLXX,QGSP_INCLXX_HP}
    # {QGSP_BERT_HP,QGSP_BIC_HP,QGSP_INCLXX_HP}
    echo -e "'run.C(\"${OPERATION}\", \"${OUTDIR}\", \""{Geant3,QGSP_BERT_HP,QGSP_BIC_HP,QGSP_INCLXX_HP}"\", "{110,250}")'\n" | xargs -I CMD --max-procs 32 bash -c 'C='"'"'CMD'"'"'; echo `date --rfc-3339=seconds` ${C} && shopt -s extglob; nice -n 19 root -l -q -b "${C}" &> '${OUTDIR}'/${C//+([^a-zA-Z0-9])/_}.log'
fi

if [ "${OPERATION}" == "digi" ]; then
    echo -e "'run.C(\"${OPERATION}\", \"${OUTDIR}\", \""{Geant3,QGSP_BERT_HP,QGSP_BIC_HP,QGSP_INCLXX_HP}"\", "{110,250}", "{20..300..5}")'\n" | xargs -I CMD --max-procs 32 bash -c 'C='"'"'CMD'"'"'; echo `date --rfc-3339=seconds` ${C} && shopt -s extglob; nice -n 19 root -l -q -b "${C}" &> '${OUTDIR}'/${C//+([^a-zA-Z0-9])/_}.log'
fi
