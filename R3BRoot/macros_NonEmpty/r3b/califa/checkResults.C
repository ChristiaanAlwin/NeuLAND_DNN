...
void checkResults()
{
    ...
    Double_t Eproj = 1.00;        // Gamma Energy in projectile frame in MeV
    ...
    Double_t minThetaiPhos = 0., maxThetaiPhos = 0.;

    if (!strcmp(calVersion, "2020"))
    {
        cout << "Using CALIFA 2020" << endl;
        minThetaBarrel = 43.16;
        maxThetaBarrel = 135.35;
        minThetaiPhos = 15.;
        maxThetaiPhos = 43.15;
        BARREL = kTRUE;
        IPHOS = kTRUE;
        ...
        cout << "Error: Calorimeter version not correctly defined! " << endl;
    ...
    TH1F* h2_Cry = new TH1F("h2_Cry", "Crystal Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    TH1F* h1_Cal = new TH1F("h1_Cal", "Nb of Crystals in hit", 20, 0, 20);
    TH1F* h2_Cal = new TH1F("h2_Cal", "Hit Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    ...
    TH1F* h2_CC = new TH1F("h2_CC",
                           "Reconstructed (Lorenzt corrected) Primary Energy (MeV)",
                           2000,
                           0,
                           2 * maxE); // Change this maximum energy
    ...
    TH2F* h2_CC2 = new TH2F(
        "h2_CC2", "Energy reconstructed vs. polar angle", 50, 0, 3.2, 400, 0, 2 * maxE); // Change this maximum energy
    ...
    TH1F* h2_Cry_barrel =
        new TH1F("h2_Cry_barrel", "Crystal Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    TH1F* h1_Cal_barrel = new TH1F("h1_Cal_barrel", "Nb of Crystals in hit", 20, 0, 20);
    TH1F* h2_Cal_barrel = new TH1F("h2_Cal_barrel", "Hit Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    ...
    TH1F* h2_CC_barrel = new TH1F("h2_CC_barrel",
                                  "Reconstructed (Lorenzt corrected) Primary Energy (MeV)",
                                  2000,
                                  0,
                                  maxE); // Change this maximum energy
    ...
    // IPHOS
    TH1F* h1_Cry_iPhos = new TH1F("h1_Cry_iPhos", "Crystal ID", 5850, 0, 5850);
    TH1F* h1_Cry_count_iPhos = new TH1F("h1_Cry_count_iPhos", "Crystal ID_count", 2, 0, 3840);
    TH1F* h2_Cry_iPhos =
        new TH1F("h2_Cry_iPhos", "Crystal Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    TH1F* h1_Cal_iPhos = new TH1F("h1_Cal_iPhos", "Nb of Crystals in hit", 20, 0, 20);
    TH1F* h2_Cal_iPhos = new TH1F("h2_Cal_iPhos", "Hit Energy (MeV)", 200, 0, 2 * maxE); // Change this maximum energy
    TH1F* h3_Cal_iPhos = new TH1F("h3_Cal_iPhos", "Hit Theta", 200, 0, 3.2);
    TH1F* h4_Cal_iPhos = new TH1F("h4_Cal_iPhos", "Hit Phi", 200, -3.2, 3.2);
    TH1F* h1_TMul_iPhos = new TH1F("h1_TMul_iPhos", "Multiplicities", 20, 0, 20);
    TH1F* h1_CryMul_iPhos = new TH1F("h1_CryMul_iPhos", "Multiplicities", 20, 0, 20);
    TH1F* h1_CalMul_iPhos = new TH1F("h1_CalMul_iPhos", "Multiplicities", 20, 0, 20);
    TH1F* h2_CC_iPhos = new TH1F("h2_CC_iPhos",
                                 "Reconstructed (Lorenzt corrected) Primary Energy (MeV)",
                                 2000,
                                 0,
                                 maxE); // Change this maximum energy
    TH1F* h3_CC_iPhos = new TH1F("h3_CC_iPhos", "Theta residuals if(caloHitsPerEvent==1)", 400, -0.25, 0.25);
    TH1F* h4_CC_iPhos = new TH1F("h4_CC_iPhos", "Phi residuals if(caloHitsPerEvent==1)", 400, -0.25, 0.25);
    ...
    Int_t crystalHitsPerEvent_iPhos = 0;
    Int_t caloHitsPerEvent_barrel = 0;
    Int_t caloHitsPerEvent_iPhos = 0;

    Int_t primary = 0;
    Int_t primary_barrel = 0;
    Int_t primary_iPhos = 0;
    ...
    for (Int_t i = 0; i < nevents; i++)
    {
        ...
        for (Int_t h = 0; h < crystalHitsPerEvent; h++)
        {
            ...
            if (BARREL && crystalHit[h]->GetCrystalId() < 1953)
            { // 1952 crystals in Barrel
            ...
            if (IPHOS && crystalHit[h]->GetCrystalId() > 1952)
            {
                crystalHitsPerEvent_iPhos++;
                h1_Cry_iPhos->Fill(crystalHit[h]->GetCrystalId());
                if (crystalHit[h]->GetEnergy() * 1000 > threshold)
                    h1_Cry_count_iPhos->Fill(crystalHit[h]->GetCrystalId());
                h2_Cry_iPhos->Fill(crystalHit[h]->GetEnergy() * 1000);
            }
        }
        ...
        for (Int_t h = 0; h < MCtracksPerEvent; h++)
        {
            if (track[h]->GetMotherId() < 0)
            {
                ...
                primary++;
                if (BARREL && momentum.Theta() > minThetaBarrel * TMath::Pi() / 180.)
                    primary_barrel++;
                if (IPHOS && momentum.Theta() < maxThetaiPhos * TMath::Pi() / 180.)
                    primary_iPhos++;
                if (primary == 1)
                {
                    if (caloHitsPerEvent == 1)
                    {
                        ...
                        if (IPHOS && caloHit[h]->GetTheta() < maxThetaiPhos * TMath::Pi() / 180.)
                        {
                            h3_CC_iPhos->Fill(momentum.Theta() - caloHit[h]->GetTheta());
                            h4_CC_iPhos->Fill(momentum.Phi() - caloHit[h]->GetPhi());
                        }
                    }
        ...
        if (BARREL)
            h1_CryMul_barrel->Fill(crystalHitsPerEvent_barrel);
        if (BARREL)
            h1_CalMul_barrel->Fill(caloHitsPerEvent_barrel);
        if (IPHOS)
            h1_CryMul_iPhos->Fill(crystalHitsPerEvent_iPhos);
        if (IPHOS)
            h1_CalMul_iPhos->Fill(caloHitsPerEvent_iPhos);

        crystalHitsPerEvent_barrel = 0;
        caloHitsPerEvent_barrel = 0;
        crystalHitsPerEvent_iPhos = 0;
        caloHitsPerEvent_iPhos = 0;

        h1_TMul->Fill(primary);
        if (BARREL)
            h1_TMul_barrel->Fill(primary_barrel);
        if (IPHOS)
            h1_TMul_iPhos->Fill(primary_iPhos);
        ...
        {
            h1_User->Fill(caloHit[1]->GetTheta() - caloHit[0]->GetTheta(), caloHit[1]->GetPhi() - caloHit[0]->GetPhi());
        }
    ...
    if (IPHOS)
    {
        c2_iPhos = new TCanvas("crystalHit_iPhos", "crystalHit_iPhos", 720, 0, 720, 900);
        c2_iPhos->SetFillColor(0);
        c2_iPhos->SetFrameFillColor(0);
    }
    TCanvas* c3_iPhos;
    if (IPHOS)
    {
        c3_iPhos = new TCanvas("caloHit_iPhos", "caloHit_iPhos", 0, 435, 720, 900);
        c3_iPhos->SetFillColor(0);
        c3_iPhos->SetFrameFillColor(0);
    }
    TCanvas* c4_iPhos;
    if (IPHOS)
    {
        c4_iPhos = new TCanvas("correlations_iPhos", "correlations_iPhos", 720, 435, 720, 900);
        c4_iPhos->SetFillColor(0);
        c4_iPhos->SetFrameFillColor(0);
    }
    ...
    if (IPHOS)
    {
        line1->DrawLine(minThetaiPhos * TMath::Pi() / 180., 0., minThetaiPhos * TMath::Pi() / 180., 10000);
        line1->DrawLine(maxThetaiPhos * TMath::Pi() / 180., 0., maxThetaiPhos * TMath::Pi() / 180., 10000);
    }
    c1->cd(4);
    h4_T->Draw();
    c1->cd();
    ...
    if (IPHOS)
    {
        line2->DrawLine(minThetaiPhos * TMath::Pi() / 180., 0., minThetaiPhos * TMath::Pi() / 180., 10000);
        line2->DrawLine(maxThetaiPhos * TMath::Pi() / 180., 0., maxThetaiPhos * TMath::Pi() / 180., 10000);
    }
    ...
    TLatex Tl;
    Tl.SetTextSize(0.06);
    Tl.DrawLatex(12, 10000, "Primary");
    Tl.SetTextSize(0.06);
    Tl.SetTextColor(2);
    Tl.DrawLatex(12, 3500, "Crystal");
    Tl.SetTextSize(0.06);
    Tl.SetTextColor(4);
    Tl.DrawLatex(12, 1000, "Calorimeter");

    Double_t mean = 0., sigma = 0., sigmaPol = 0.;
    Double_t mean_barrel = 0., sigma_barrel = 0.;
    Double_t mean_iPhos = 0., sigma_iPhos = 0.;
    ...
    if (myfunc != NULL)
        mean = myfunc->GetParameter(1); // value of 2st parameter (mean)
    if (myfunc != NULL)
        sigma = myfunc->GetParameter(2); // value of 2st parameter (sigma)
    ...
    if (myfuncPol != NULL)
        sigmaPol = myfuncPol->GetParameter(2); // value of 2st parameter (mean)
    ...
    c6_2->cd();
    title =
        new TLatex(0.01, 0.2, "Calorimeter Hits: checking the resolution of the reconstruction vs. the polar angle");
    title->SetTextSize(0.6);
    title->Draw();
    c6->cd();
    ...
    if (BARREL)
    {
        ...
        if (IPHOS)
        {
            line2_barrel->DrawLine(minThetaiPhos * TMath::Pi() / 180., 0., minThetaiPhos * TMath::Pi() / 180., 10000);
            line2_barrel->DrawLine(maxThetaiPhos * TMath::Pi() / 180., 0., maxThetaiPhos * TMath::Pi() / 180., 10000);
        }
        ...
        h1_TMul_barrel->Draw();
        h1_CalMul_barrel->Draw("SAME");
        h1_CryMul_barrel->Draw("SAME");
        c4_barrel->cd(1)->SetLogy();

        TLatex Tl_barrel;
        Tl_barrel.SetTextSize(0.06);
        Tl_barrel.DrawLatex(12, 10000, "Primary");
        Tl_barrel.SetTextSize(0.06);
        Tl_barrel.SetTextColor(2);
        Tl_barrel.DrawLatex(12, 3500, "Crystal");
        Tl_barrel.SetTextSize(0.06);
        Tl_barrel.SetTextColor(4);
        Tl_barrel.DrawLatex(12, 1000, "Calorimeter");

        c4_barrel->cd(2);
        h2_CC_barrel->Fit("gaus", "", "", Eproj - 0.05 * Eproj, Eproj + 0.05 * Eproj);
        // getting the value of the fit parameters
        TF1* myfunc_barrel = h2_CC_barrel->GetFunction("gaus");
        if (myfunc_barrel != NULL)
            mean_barrel = myfunc_barrel->GetParameter(1); // value of 2st parameter (mean)
        if (myfunc_barrel != NULL)
            sigma_barrel = myfunc_barrel->GetParameter(2); // value of 2st parameter (mean)
    ...
    if (IPHOS)
    {
        ...
        c2_iPhos->cd(2);
        h2_Cry_iPhos->Draw();
        c2_iPhos->cd(2)->SetLogy();
        c2_iPhos->cd();
        TPad* c2_2_iPhos = new TPad("c2_2_iPhos", "c2_2_iPhos", 0., 0., 0.9, 0.03);
        c2_2_iPhos->Draw();
        c2_2_iPhos->cd();
        title = new TLatex(0.1, 0.2, "Crystal Hits [IPHOS]: information in the CALIFA crystals");
        title->SetTextSize(0.6);
        title->Draw();
        c2_iPhos->cd();

        // CALORIMETER HIT CANVAS -- ONLY IPHOS!!!
        c3_iPhos->cd();
        c3_iPhos->Divide(2, 2);
        c3_iPhos->cd(1);
        h1_Cal_iPhos->Draw();
        c3_iPhos->cd(2);
        h2_Cal_iPhos->Draw();
        c3_iPhos->cd(2)->SetLogy();
        c3_iPhos->cd(3);
        h3_Cal_iPhos->Draw();

        TLine* line2_iPhos = new TLine(0, 0, 0, 0);
        line2_iPhos->SetLineStyle(2);
        line2_iPhos->Draw();
        line2_iPhos->DrawLine(minThetaBarrel * TMath::Pi() / 180., 0, minThetaBarrel * TMath::Pi() / 180., 10000);
        line2_iPhos->DrawLine(maxThetaBarrel * TMath::Pi() / 180., 0., maxThetaBarrel * TMath::Pi() / 180., 10000);
        if (BARREL)
        {
            line2->DrawLine(minThetaBarrel * TMath::Pi() / 180., 0, minThetaBarrel * TMath::Pi() / 180., 10000);
            line2->DrawLine(maxThetaBarrel * TMath::Pi() / 180., 0., maxThetaBarrel * TMath::Pi() / 180., 10000);
        ...
        h4_Cal_iPhos->Draw();
        c3_iPhos->cd();
        TPad* c3_2_iPhos = new TPad("c3_2_iPhos", "c3_2_iPhos", 0., 0., 0.9, 0.03);
        c3_2_iPhos->Draw();
        c3_2_iPhos->cd();
        title = new TLatex(0.1, 0.2, "Calorimeter Hits [IPHOS]: reconstruction from the crystals using the HitFinder");
        title->SetTextSize(0.6);
        title->Draw();
        c3_iPhos->cd();

        // CORRELATIONS CANVAS -- ONLY IPHOS!!!
        c4_iPhos->cd();
        c4_iPhos->Divide(2, 2);
        h1_CryMul_iPhos->SetLineColor(kRed);
        h1_CalMul_iPhos->SetLineColor(kBlue);
        c4_iPhos->cd(1);
        h1_TMul_iPhos->GetYaxis()->SetRangeUser(1, 2 * totalEvents * multiplicity);
        h1_TMul_iPhos->Draw();
        h1_CalMul_iPhos->Draw("SAME");
        h1_CryMul_iPhos->Draw("SAME");
        c4_iPhos->cd(1)->SetLogy();

        TLatex Tl_iPhos;
        Tl_iPhos.SetTextSize(0.06);
        Tl_iPhos.DrawLatex(12, 10000, "Primary");
        Tl_iPhos.SetTextSize(0.06);
        Tl_iPhos.SetTextColor(2);
        Tl_iPhos.DrawLatex(12, 3500, "Crystal");
        Tl_iPhos.SetTextSize(0.06);
        Tl_iPhos.SetTextColor(4);
        Tl_iPhos.DrawLatex(12, 1000, "Calorimeter");

        c4_iPhos->cd(2);
        h2_CC_iPhos->Fit("gaus", "", "", Eproj - 0.05 * Eproj, Eproj + 0.05 * Eproj);
        // getting the value of the fit parameters
        TF1* myfunc_iPhos = h2_CC_iPhos->GetFunction("gaus");
        if (myfunc_iPhos != NULL)
            mean_iPhos = myfunc_iPhos->GetParameter(1); // value of 2st parameter (mean)
        if (myfunc_iPhos != NULL)
            sigma_iPhos = myfunc_iPhos->GetParameter(2); // value of 2st parameter (mean)
        cout << "TESTING mean_iPhos: " << mean_iPhos << ",  sigma_iPhos: " << sigma_iPhos << endl;
        h2_CC_iPhos->Draw();
        c4_iPhos->cd(2)->SetLogy();
        c4_iPhos->cd(3);
        gStyle->SetOptStat(1);
        gStyle->SetOptFit(1);
        h3_CC_iPhos->Fit("gaus", "", "", -0.03, 0.03);
        h3_CC_iPhos->Draw();
        c4_iPhos->cd(4);
        gStyle->SetOptStat(1);
        gStyle->SetOptFit(1);
        h4_CC_iPhos->Fit("gaus", "", "", -0.06, 0.06);
        h4_CC_iPhos->Draw();
        c4_iPhos->cd();
        TPad* c4_2_iPhos = new TPad("c4_2_iPhos", "c4_2_iPhos", 0., 0., 0.9, 0.03);
        c4_2_iPhos->Draw();
        c4_2_iPhos->cd();
        title = new TLatex(0.1, 0.2, "Calorimeter Hits[IPHOS]: checking the resolution of the reconstruction");
        title->SetTextSize(0.6);
        title->Draw();
        c4_iPhos->cd();
    }
    ...
    if (0)
    {
        ...
        mean_iPhos = 4.94582e+00;   // Mean of energy peak fit
        sigma_iPhos = 1.16033e-01;  // Sigma of energy peak fit
    ...
    Double_t particlesIniPhos = h3_T->Integral(minThetaiPhos, maxThetaiPhos);    // angular coverture of IPHOS
    ...
    Double_t photopeakParticles2Sigma_barrel = 0., photopeakParticles2Sigma_iPhos = 0.;
    Double_t photopeakParticles3Sigma_barrel = 0., photopeakParticles3Sigma_iPhos = 0.;
    if (BARREL)
    {
    ...
    if (IPHOS)
    {
        Double_t resolution_iPhos = (2.35 * sigma_iPhos * 100) / mean_iPhos;
        Int_t binLeft2Sigma_iPhos = h2_CC_iPhos->FindBin(mean_iPhos - 2 * sigma_iPhos);
        Int_t binRight2Sigma_iPhos = h2_CC_iPhos->FindBin(mean_iPhos + 2 * sigma_iPhos);
        photopeakParticles2Sigma_iPhos = h2_CC_iPhos->Integral(binLeft2Sigma_iPhos, binRight2Sigma_iPhos);
        Int_t binLeft3Sigma_iPhos = h2_CC_iPhos->FindBin(mean_iPhos - 3 * sigma_iPhos);
        Int_t binRight3Sigma_iPhos = h2_CC_iPhos->FindBin(mean_iPhos + 3 * sigma_iPhos);
        photopeakParticles3Sigma_iPhos = h2_CC_iPhos->Integral(binLeft3Sigma_iPhos, binRight3Sigma_iPhos);
    }

    cout << endl
         << endl
         << "CALIFA version: " << calVersion << "      Particle energy: " << Eproj << "      Events:  " << totalEvents
         << endl;
    cout << "Multiplicity: " << multiplicity << " particles/event" << endl;
    cout << "Threshold: " << threshold << " MeV" << endl;
    cout << "Exp. resolution: " << ExpRes << "%" << endl;
    cout << "Particles emitted: totalEvents * multiplicity = " << totalEvents * multiplicity << " total particles "
         << endl
         << endl;
    cout << "Total crystals with signal: " << h1_Cry->GetEntries()
         << "  above threshold: " << h1_Cry_count->GetEntries() << endl;
    ...
    if (BARREL)
    {
        cout << "Particles in the Barrel = " << particlesInBarrel << " between angles " << minThetaBarrel << " and "
             << maxThetaBarrel << endl;
        cout << "Mean number of crystals with signal in Barrel: " << h1_CryMul_barrel->GetMean() << endl;
        cout << "Mean number of crystals with signal in Barrel per primary: "
             << h1_CryMul_barrel->Integral(2, 50) / (Double_t)(totalEvents * multiplicity) << endl;
        cout << "Mean number of crystals with signal in Barrel per primary in Barrel: "
             << (Double_t)h1_CryMul_barrel->GetMean() * totalEvents * multiplicity / (Double_t)particlesInBarrel
             << endl;
        cout << "Mean number of calorimeter hits in Barrel: " << h1_CalMul_barrel->GetMean() << endl;
        cout << "Mean number of calorimeter hits in Barrel per primary: "
             << h1_CalMul_barrel->Integral(2, 50) / (Double_t)(totalEvents * multiplicity) << endl;
        cout << "Mean number of calorimeter hits in Barrel per primary in Barrel: "
             << (Double_t)h1_CalMul_barrel->GetMean() * totalEvents * multiplicity / (Double_t)particlesInBarrel
             << endl;
        cout << "Particles in barrel / all particles: "
             << (Double_t)particlesInBarrel * 100 / (Double_t)(totalEvents * multiplicity) << "%" << endl
             << endl;
    }
    if (IPHOS)
    {
        cout << "Particles in the iPhos = " << particlesIniPhos << " between angles " << minThetaiPhos << " and "
             << maxThetaiPhos << endl;
        cout << "Mean number of crystals with signal in iPhos: " << h1_CryMul_iPhos->GetMean() << endl;
        cout << "Mean number of crystals with signal in iPhos per primary: "
             << h1_CryMul_iPhos->Integral(2, 50) / ((Double_t)(totalEvents * multiplicity)) << endl;
        cout << "Mean number of crystals with signal in iPhos per primary in iPhos: "
             << (Double_t)h1_CryMul_iPhos->GetMean() * totalEvents * multiplicity / (Double_t)particlesIniPhos << endl;
        cout << "Mean number of calorimeter hits in iPhos: " << h1_CalMul_iPhos->GetMean() << endl;
        cout << "Mean number of calorimeter hits in iPhos per primary: "
             << h1_CalMul_iPhos->Integral(2, 50) / (Double_t)(totalEvents * multiplicity) << endl;
        cout << "Mean number of calorimeter hits in iPhos per primary in iPhos: "
             << (Double_t)h1_CalMul_iPhos->GetMean() * (Double_t)totalEvents * multiplicity / (Double_t)particlesIniPhos
             << endl;
        cout << "Particles in iPhos / all particles: "
             << (Double_t)particlesIniPhos * 100 / (Double_t)(totalEvents * multiplicity) << "%" << endl
             << endl;
    }
    ...
    cout << "Photopeak efficiency: Np(2sigma)/particles (%) = "
         << (Double_t)photopeakParticles2Sigma * 100 / (Double_t)(totalEvents * multiplicity) << endl;
    cout << "Photopeak efficiency: Np(3sigma)/particles (%) = "
         << (Double_t)photopeakParticles3Sigma * 100 / (Double_t)(totalEvents * multiplicity) << endl;
    if (BARREL)
        cout << "Photopeak efficiency: Np(2sigma)/particlesInBarrel (%) = "
             << (Double_t)photopeakParticles2Sigma * 100 / (Double_t)particlesInBarrel << endl;
    if (BARREL)
        cout << "Photopeak efficiency: Np(3sigma)/particlesInBarrel (%) = "
             << (Double_t)photopeakParticles3Sigma * 100 / (Double_t)particlesInBarrel << endl
             << endl
             << endl;
    if (IPHOS)
        cout << "Photopeak efficiency: Np(2sigma)/particlesIniPhos (%) = "
             << (Double_t)photopeakParticles2Sigma * 100 / (Double_t)particlesIniPhos << endl;
    if (IPHOS)
        cout << "Photopeak efficiency: Np(3sigma)/particlesIniPhos (%) = "
             << (Double_t)photopeakParticles3Sigma * 100 / (Double_t)particlesIniPhos << endl
             << endl
             << endl;
    ...
    sprintf(oT3,
            "%s%i%s",
            "Particles emitted: totalEvents * multiplicity = ",
            totalEvents * multiplicity,
            " total particles ");
    sprintf(oT4,
            "%s%f%s%f",
            "Total crystals with signal: ",
            h1_Cry->GetEntries(),
            "  above threshold: ",
            h1_Cry_count->GetEntries());
    ...
    if (BARREL)
        sprintf(oT8,
                ...
                maxThetaBarrel);
    if (BARREL)
        sprintf(oT9, "%s%f", "Mean number of crystals with signal in Barrel: ", h1_CryMul_barrel->GetMean());
    if (BARREL)
        sprintf(oT10,
                "%s%f",
                "Mean number of crystals with signal in Barrel per primary: ",
                (Double_t)h1_CryMul_barrel->Integral(2, 50) / (Double_t)(totalEvents * multiplicity));
    if (BARREL)
        sprintf(oT11,
                "%s%f",
                "Mean number of crystals with signal in Barrel per primary in Barrel: ",
                (Double_t)h1_CryMul_barrel->GetMean() * totalEvents * multiplicity / (Double_t)particlesInBarrel);
    if (BARREL)
        sprintf(oT12, "%s%f", "Mean number of calorimeter hits in Barrel: ", h1_CalMul_barrel->GetMean());
    if (BARREL)
        sprintf(oT13,
                "%s%f",
                "Mean number of calorimeter hits in Barrel per primary: ",
                (Double_t)h1_CalMul_barrel->Integral(2, 50) / (Double_t)(totalEvents * multiplicity));
    if (BARREL)
        sprintf(oT14,
                "%s%f",
                "Mean number of calorimeter hits in Barrel per primary in Barrel: ",
                (Double_t)h1_CalMul_barrel->GetMean() * totalEvents * multiplicity / (Double_t)particlesInBarrel);
    if (BARREL)
        sprintf(oT15,
                "%s%f",
                "Particles in barrel / all particles (%): ",
                (Double_t)particlesInBarrel * 100 / (Double_t)(totalEvents * multiplicity));

    if (IPHOS)
        sprintf(oT16,
                "%s%f%s%f%s%f",
                "Particles in the iPhos: ",
                particlesIniPhos,
                " between angles ",
                minThetaiPhos,
                " and ",
                maxThetaiPhos);
    if (IPHOS)
        sprintf(oT17, "%s%f", "Mean number of crystals with signal in iPhos: ", h1_CryMul_iPhos->GetMean());
    if (IPHOS)
        sprintf(oT18,
                "%s%f",
                "Mean number of crystals with signal in iPhos per primary: ",
                (Double_t)h1_CryMul_iPhos->Integral(2, 50) / (Double_t)(totalEvents * multiplicity));
    if (IPHOS)
        sprintf(oT19,
                "%s%f",
                "Mean number of crystals with signal in iPhos per primary in iPhos: ",
                (Double_t)h1_CryMul_iPhos->GetMean() * totalEvents * multiplicity / (Double_t)particlesIniPhos);
    if (IPHOS)
        sprintf(oT20, "%s%f", "Mean number of calorimeter hits in iPhos: ", h1_CalMul_iPhos->GetMean());
    if (IPHOS)
        sprintf(oT21,
                "%s%f",
                "Mean number of calorimeter hits in iPhos per primary: ",
                (Double_t)h1_CalMul_iPhos->Integral(2, 50) / (Double_t)(totalEvents * multiplicity));
    if (IPHOS)
        sprintf(oT22,
                "%s%f",
                "Mean number of calorimeter hits in iPhos per primary in iPhos: ",
                (Double_t)h1_CalMul_iPhos->GetMean() * totalEvents * multiplicity / (Double_t)particlesIniPhos);
    if (IPHOS)
        sprintf(oT23,
                "%s%f",
                "Particles in iPhos / all particles (%): ",
                (Double_t)particlesIniPhos * 100 / (Double_t)(totalEvents * multiplicity));

    sprintf(oT24, "%s", "Photopeak efficiencies");
    sprintf(oT25, "%s%f", "Photopeak entries (+-2 sigmas, 95.44%): ", photopeakParticles2Sigma);
    sprintf(oT26,
            "%s%f",
            "Photopeak entries/particles (%): ",
            (Double_t)photopeakParticles2Sigma * 100 / (Double_t)(totalEvents * multiplicity));

    if (BARREL)
        sprintf(oT27, "%s", "Photopeak efficiencies (BARREL)");
    if (BARREL)
        sprintf(oT28, "%s%f", "Photopeak entries (+-2 sigmas, 95.44%): ", photopeakParticles2Sigma_barrel);
    if (BARREL)
        sprintf(oT29,
                "%s%f",
                "Photopeak entries/particles (%): ",
                photopeakParticles2Sigma_barrel * 100 / (Double_t)(totalEvents * multiplicity));

    if (IPHOS)
        sprintf(oT30, "%s", "Photopeak efficiencies (IPHOS)");
    if (IPHOS)
        sprintf(oT31, "%s%f", "Photopeak entries (+-2 sigmas 95.44%): ", photopeakParticles2Sigma_iPhos);
    if (IPHOS)
        sprintf(oT32,
                "%s%f",
                "Photopeak entries/particles (%): ",
                (Double_t)photopeakParticles2Sigma_iPhos * 100 / (Double_t)(totalEvents * multiplicity));

    sprintf(oT33, "%s", "Photopeak and angle resolution");
    sprintf(oT34, "%s%f", "FWHM (%): ", sigma * 235. / mean);
    if (BARREL)
        sprintf(oT35, "%s%f", "FWHM BARREL (%): ", sigma_barrel * 235. / mean_barrel);
    if (IPHOS)
        sprintf(oT36, "%s%f", "FWHM IPHOS (%): ", sigma_iPhos * 235. / mean_iPhos);
    sprintf(
        oT37, "%s%f%s%f", "Polar angle resolution (sigma in rads, degs): ", sigmaPol, ", ", sigmaPol * 180 / 3.141597);
    ...
    if (BARREL)
        l.DrawLatex(0.08, 0.85, oT4);
    if (IPHOS)
        l.DrawLatex(0.08, 0.82, oT5);
    l.DrawLatex(0.08, 0.79, oT6);
    l.DrawLatex(0.08, 0.76, oT7);

    if (BARREL)
        l.DrawLatex(0.08, 0.71, oT8);
    if (BARREL)
        l.DrawLatex(0.08, 0.69, oT9);
    if (BARREL)
        l.DrawLatex(0.08, 0.67, oT10);
    if (BARREL)
        l.DrawLatex(0.08, 0.65, oT11);
    if (BARREL)
        l.DrawLatex(0.08, 0.63, oT12);
    if (BARREL)
        l.DrawLatex(0.08, 0.61, oT13);
    if (BARREL)
        l.DrawLatex(0.08, 0.59, oT14);
    if (BARREL)
        l.DrawLatex(0.08, 0.57, oT15);

    if (IPHOS)
        l.DrawLatex(0.08, 0.54, oT16);
    if (IPHOS)
        l.DrawLatex(0.08, 0.52, oT17);
    if (IPHOS)
        l.DrawLatex(0.08, 0.50, oT18);
    if (IPHOS)
        l.DrawLatex(0.08, 0.48, oT19);
    if (IPHOS)
        l.DrawLatex(0.08, 0.46, oT20);
    if (IPHOS)
        l.DrawLatex(0.08, 0.44, oT21);
    if (IPHOS)
        l.DrawLatex(0.08, 0.42, oT22);
    if (IPHOS)
        l.DrawLatex(0.08, 0.40, oT23);
    ...
    if (BARREL)
        l.DrawLatex(0.08, 0.29, oT27);
    if (BARREL)
        l.DrawLatex(0.08, 0.27, oT28);
    if (BARREL)
        l.DrawLatex(0.08, 0.25, oT29);

    if (IPHOS)
        l.DrawLatex(0.08, 0.22, oT30);
    if (IPHOS)
        l.DrawLatex(0.08, 0.20, oT31);
    if (IPHOS)
        l.DrawLatex(0.08, 0.18, oT32);

    l.DrawLatex(0.08, 0.15, oT33);
    l.DrawLatex(0.08, 0.13, oT34);
    if (BARREL)
        l.DrawLatex(0.08, 0.11, oT35);
    if (IPHOS)
        l.DrawLatex(0.08, 0.09, oT36);
    ...
    c2->Print("output.ps");
    if (BARREL)
        c2_barrel->Print("output.ps");
    if (IPHOS)
        c2_iPhos->Print("output.ps");
    c3->Print("output.ps");
    if (BARREL)
        c3_barrel->Print("output.ps");
    if (IPHOS)
        c3_iPhos->Print("output.ps");
    c4->Print("output.ps");
    if (BARREL)
        c4_barrel->Print("output.ps");
    if (IPHOS)
        c4_iPhos->Print("output.ps");
    c6->Print("output.ps)");
}
