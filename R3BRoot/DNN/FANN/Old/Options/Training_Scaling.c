...
int main()
{
    ...
    int const MultSize = 5;    // max. multiplicity to look for.
    ...
    fann_train_on_file(ann,"./DataSets/DNN_TrainingFile4.txt",200,10,1e-6);
    // 1) the network.
    ...
    double Output;
    double Maximum;
    double StepSize = 1.0/((int) MultSize);
    double kdouble;
    int Calculated_Multiplicity[MultSize+1];
    int DataSet_Multiplicity[MultSize+1];
    int Index;
    int TrueMultIndex;
	...
	struct fann_train_data* data = fann_read_train_from_file("./DataSets/DNN_VerificationFile4.txt");
	...
	for(int i = 0; i < fann_length_train_data(data); i++)
	{
        ...
        if (i<20)
        {
            printf("Test (%f, %f) -> %f, should be %f\n",data->input[i][0], data->input[i][1], calc_out[0], data->output[i][0]);
        }
        
        // Count true multiplicity:
        for (int k = 0; k<(MultSize+1); ++k)
        {
            kdouble = (int) k;
            kdouble = kdouble*StepSize;
            
            if ((data->output[i][0]>=(kdouble-0.5*StepSize))&&(data->output[i][0]<(kdouble+0.5*StepSize)))
            {
                Index = k;
                TrueMultIndex = k;
            }
        }
        DataSet_Multiplicity[Index] = DataSet_Multiplicity[Index] + 1;
        ...
        for (int k = 0; k<(MultSize+1); ++k)
        {
            ...
            if ((calc_out[0]>=(kdouble-0.5*StepSize))&&(calc_out[0]<(kdouble+0.5*StepSize)))
            {
                Index = k;
            }
        }
        ...
        {
            if (Index==TrueMultIndex) {Calculated_Multiplicity[Index] = Calculated_Multiplicity[Index] + 1;}
        }
        else
        {
