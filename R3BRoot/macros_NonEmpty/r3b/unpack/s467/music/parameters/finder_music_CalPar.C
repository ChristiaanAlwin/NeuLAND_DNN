...
void finder_music_CalPar()
{
    ...
    // MUSIC output file with pedestals and position calibration parameters
    TString outputCalFile = "./Music_CalParam.par";
    ...
    if (expId == 444)
    {
        // ucesb_path = "/u/land/lynx.landexp/202002_s444/upexps/202002_s444/202002_s444 --allow-errors
        // --input-buffer=100Mi";
    ...
    // Please, select mwpc1 or mwpc2 for the angle calibration!!
    ...
    // Selection of second MWPC for angle calibration
    TString namedet;
    if (fMwpc2)
        namedet = "Mwpc2";
    else if (fMwpc1)
    ...
    R3BMusicMapped2CalPar* MusMap2CalPar = new R3BMusicMapped2CalPar("Angle-XZ calibrator", 1, "Mwpc0", namedet);
    MusMap2CalPar->SetPosMwpcA(PosZ_MWPC0);
    MusMap2CalPar->SetPosMwpcB(PosZ_MWPC2);
    MusMap2CalPar->SetPosMusic(PosZ_Music);
    MusMap2CalPar->SetFitLimits(12000.,23000.);
